...
```bash
APK=$(ls -t ~/tmp/shiroikuma-mpvkakucho_*.apk 2>/dev/null | head -1)
VERSION=$(basename "$APK" | sed -E 's/^shiroikuma-mpvkakucho_(.+)_arm64-v8a\.apk$/\1/')   # e.g. 1.2.9+1
...
> **The tag format matters here.** The in-app updater (`standard` flavor) reads this repo's
> `releases/latest` and compares tag names against the running `versionName`. Keep the tag exactly
> equal to the fork `versionName` (`<upstream>+<N>`, no `v`) so the updater never offers a "newer"
> version that is really the one already installed.
...
1. **Keep the GitHub default branch on `custom`** so the repo page lands on our README, not
   ...
   ```bash
   gh repo edit ShiroiKuma0/shiroikuma-mpvkakucho --default-branch custom
   gh repo edit ShiroiKuma0/shiroikuma-mpvkakucho \
     --description "白い熊 mpv拡張 — a fork of mpvEx: an mpv-powered video player for Android, de-branded, black-yellow, side-by-side installable. Apache-2.0."
   ```

2. **Update the README badge** — point the "Latest release" line at the new version.

3. **Update `CHANGELOG-shiroikuma.md`.** Upstream owns its own release notes; **our** fork changelog
   is `CHANGELOG-shiroikuma.md` — never fold fork notes into an upstream file. Keep it **specific**:
   ...
   ```
   Group by area (playback / subtitles / browser & network / look / fixes), one specific bullet each
...
5. **Tag and release.** Annotated tag at `HEAD`, then a GitHub release targeting `custom` with the
   APK attached. **Always pin the repo with `-R ShiroiKuma0/shiroikuma-mpvkakucho`** — the working
   copy has an `upstream` remote (`marlboro-advance/mpvEx`), and a bare `gh release` will otherwise
   404 against upstream. Write the notes to a real file under `~/tmp` (do **not** rely on `$TMPDIR`,
   which is unset when the sandbox is off):
   ```bash
   REPO=ShiroiKuma0/shiroikuma-mpvkakucho
   git tag -a "$TAG" -m "白い熊 mpv拡張 $VERSION"
   git push origin "$TAG"
   NOTES="$HOME/tmp/mpvkakucho_release_notes.md"
   sed -n "/^## ${VERSION} —/,/^## [0-9]/p" CHANGELOG-shiroikuma.md | sed '/^## [0-9]/d' | tail -n +2 > "$NOTES"
   gh release create "$TAG" "$APK" -R "$REPO" \
     --target custom \
     --title "白い熊 mpv拡張 $VERSION" \
     --notes-file "$NOTES"
   rm -f "$NOTES"
   ```
   Keep the APK asset name as built (`shiroikuma-mpvkakucho_<VERSION>_arm64-v8a.apk`).

6. **Report** the release URL and confirm the default branch:
   ```bash
   gh release view "$TAG" -R ShiroiKuma0/shiroikuma-mpvkakucho --json url -q .url
   gh repo view ShiroiKuma0/shiroikuma-mpvkakucho --json defaultBranchRef -q .defaultBranchRef.name
   ```
...
- Upstream's own `.github/workflows/` release pipeline is **not** used — it signs with a repo secret
  we don't have and publishes 4 ABI splits. Our releases are cut by hand from this skill.
